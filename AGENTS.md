...
Place runtime code under `src/`, grouping by capability: `src/orchestrator/` for chain resolution and state services, `src/hooks/` for Claude Code hook adapters, `src/api/` for Express routes, and `src/persistence/` for Prisma repositories. Keep shared types in `src/core/` and environment helpers in `src/config/`. Tests mirror the module tree in `tests/` using `*.spec.ts` files. Generated Prisma schema and migrations live in `prisma/`. Documentation, product context, and runbooks stay under `docs/` (see `docs/PRD.md` and `docs/development-plan.md`). Agent prompt templates are managed in Claude Code’s `.claude/agents/` directory; commit only references, not the raw prompt files.
...
Store secrets in `.env` (never commit); expose typed access via `src/config/env.ts`. Use API keys for admin transitions and redact them from logs. When adding new agent roles or complexities, update configuration validation so startup checks assert all 15 role/level combinations.
